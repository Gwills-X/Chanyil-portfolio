export const testimonials = [
	{
		name: "Prophetess Dhiann Claxton",
		role: "Prophetess",
		text: "Whenever I think of Channelle, I see a woman of great faith whose heart and eyes are steadfastly fixed on Jesus Christ and the things that concern Him. She trusts, honors, and loves Him deeply, and this devotion reflects clearly in her life and ministry. I truly admire and commend her for this and for the many other beautiful qualities she embodies.",
	},

	{
		name: "Mrs. Phoebe Caleb Manabe",
		role: "Northern Nigeria",
		text: "Channelle, the founder of Chayil Daughters, has been a true source of inspiration to me. Through her obedience and surrender to the Lord, God has used this platform to transform lives. Being part of Chayil Daughters has deepened my prayer life and strengthened my walk with God.",
	},

	{
		name: "Pastor Edwin Quaye",
		role: "Pastor",
		text: "Channelle’s support during the launch of my church was invaluable. She has a heart for ministry and has consistently helped others grow spiritually across nations. She truly makes a difference in every life she touches.",
	},

	{
		name: "Miriam",
		role: "Author",
		text: "Channelle is a dedicated leader with a deep heart for women. She creates safe spaces for growth and consistently points people back to God.",
	},
	{
		name: "Hannibel",
		role: "Mentee",
		text: "Prophetess Channelle Chayil is a true spiritual mother. Her prayers, guidance, and mentorship have transformed my life.",
	},
	{
		name: "Pastor Augustine Aboagye",
		role: "PAAMTV UK",
		text: " Channel is not only talented vocally and musically, but she is also a true servant leader. She prepares diligently, leads with excellence, and creates an atmosphere where others feel encouraged to engage freely in worship. Over the years, I have seen her remain faithful, teachable, and committed, even in challenging seasons. What stands out most about Channel is her heart for people. She leads with grace, integrity, and love, making those around her feel valued and inspired. I wholeheartedly recommend Channel White as a worship leader, confident that she will continue to impact lives and lead others into powerful encounters with God.",
	},
	{
		name: "Dr. Ferdinard Senyo Lawson (MRSPH)",
		role: " BA Public Health and social care",
		text: " Channelle excels at communication and collaboration, always bringing innovative solutions and a positive attitude to complex challenges. Highly recommend! I consistently admire Channelle’ s dedication and ability to turn complex problems into manageable tasks when it comes to the things of God. She is motivational, reliable, results-driven team player.  Her gift of singing and ministering during praise and worship was wonderful; I could listen and join in all day! The music is perfect, so inspiring and just right. Not only did I enjoy her ministration, but the presence of God was amazing when she ministered. The atmosphere was spirit filled.  I was really blessed by the song Channelle sang. God has bestowed upon her a great gift. Recently, Channelle was in the Northen part of Ghana sharing and ministering the word of God and supporting the women in that region. ",
	},
	{
		name: "Lady Pastor Magdalene Seidu",
		role: "",
		text: " The life of Woman of God Channelle Chayil has taught me to trust God in every situation. Her journey from divorce to a beautifully restored marriage is a powerful testimony of God’s faithfulness. I deeply admire her heart for women across the globe, widows, and children without parents. As a woman of God, wife, mother, leader, and mentor, she leads with grace and purpose. Her mentorship is truly unmatched, yet she remains a loving mother to Levi and a blessed wife to her husband. Her life continually inspires faith, hope, and unwavering trust in God. ",
	},
	{
		name: "Dr. Stephen Aderinto",
		role: "Husband",
		text: "Channelle is my wife and a devoted mother, a strong pillar of support for our family, and a woman whose love and affection are deeply felt in our home. She has selflessly upheld the home front with unwavering dedication, consistently prioritising the wellbeing of our family. She has truly been a blessing to our home. Both within and beyond our household, Channelle is a woman of genuine faith, humility, and prayer. Her life reflects a deep commitment to obeying God and serving others, a truth evident in the many good works for which she is often commended. I am fully aware of her missionary outreach efforts, and they are nothing short of admirable. In a world that frequently promotes self-focus, her heart for people shines brightly through her ministry, Chayil Daughters, as well as her work with widows and orphans and her mission efforts across Africa, where she is always eager to serve those in need. From close observation, what truly sets her apart is her selfless passion to see others succeed, especially women. She is deeply committed to helping women grow, walk confidently in their God-given purpose, and live it out fully. Channelle is a strong and trustworthy leader, and the faithfulness she demonstrates today points to an even greater impact ahead. She is truly a blessing to the world.",
	},
];
